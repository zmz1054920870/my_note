...
https://www.bilibili.com/video/BV1Sa4y1H7e3/?spm_id_from=pageDriver&vd_source=908f99a5244c42140790061d37500028
...
**VTEP**： VXLAN隧道的一个端点，它其实是一个IP地址（隧道可以理解成一个IP到另外一个IP，本端的IP就是一个VTEP）

**VNI**：就是VXLAN的一个ID（VLAN也有ID ，如果1-4094），VNI一共有1600W个

**BD**：广播域，我们知道一个VLAN就是一个广播域，VXLAN也是一样的，它的广播域用BD来表示，相当于VLAN

**vBDif**：VXLAN逻辑接口，相当于VLANIF一样

**VAP**：接入测的 access point

**NVE**：如果一台设备运行了VXLAN，我们就讲这台设备是一个NVE

![image-20220830001056855](image-20220830001056855.png)

三台物理设备设备，左边是一台接入层交换机TOR，右边也是一台接入层交换机TOR和一个网关。在三台设备上都运行VXLAN ，这三台设备都是一个NVE。每个NVE呢，都有一个VTEP，这个VTEP就是一个本地的IP地址，比如是一个loopback  ip，GW 也有一个 VTEP， NVE3也有一个VTEP。两个IP 就可以生成一个隧道，所以上面有三条隧道。

然后呢，每个设备上面，它可以创建很多个VNI， 就像VLAN一样。然后呢，所有VNI都共享同一个VTEP，然后每一个VNI都有一个自己的广播域，也就是说一个VNI对应一个BD，VNI的number和 BD的number可以不相同哈
