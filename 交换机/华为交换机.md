...
1. 选出根桥
  ...
  1. 每个交换机只要运行了STP（Spanning Tree Protocol），它就会有一个桥ID（优先级+交换机MAC），桥ID最小的交换机为根桥
  ...
  2. 当所有交换机的优先级一样的时候，MAC地址最小的为根桥，还可以通过认为指定优先级的方式指定根桥

  3. https://www.bilibili.com/video/BV1ZY4y1W7ip?p=23&spm_id_from=pageDriver&vd_source=908f99a5244c42140790061d37500028

  4. 通过`dis stp查看`,交换机默认优先级是32768，同时通过这个命令我们也可以查看到我们交换机的MAC地址

  	![image-20220803012538254](image-20220803012538254.png)

2. 选出根端口

  1. 根桥上面是不选根端口的，根端口只能是在每个非根桥上，选出一个根端口
