...
```
pip install websockert-client == 0.25.0
```



```
ws = websocket.create_connection(ws_url)	# 创建websocket
payload = {
            'id': 1,
            'method': 'Runtime.evaluate',
            'params': {'expression': js}	# jss
        }
data = json.dumps(payload)
ws.send(data)
res = ws.recv()

--remote-debugging-port=9222 --user-data-dir='~/code/chrome-test	# 这里只有一个一边单引号

"var {element} = document.evaluate('{xpath}',document.documentElement, null,XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null)"

"{element}.snapshotItem({index}).getAttribute('{attribute}')"
```